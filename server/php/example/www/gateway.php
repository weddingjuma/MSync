...
/*
 * This example script implements the fondaSMS API.
 *
 ...
 *
 * To use this file, set the URL to this file as as the the Server URL in the fondaSMS app.
 * The password in the fondaSMS app settings must be the same as $PASSWORD in config.php.
 */

require_once dirname(__DIR__)."/config.php";
require_once dirname(dirname(__DIR__))."/fondaSMS.php";

$request = fondaSMS::get_request();
...
switch ($action->type)
{
    case fondaSMS::ACTION_INCOMING:
        ...
        if ($action->message_type == fondaSMS::MESSAGE_TYPE_MMS)
        {
    ...
    case fondaSMS::ACTION_OUTGOING:
        ...
        while ($file = readdir($dir))
        {
            if (preg_match('#\.json$#', $file))
            {
                ...
                if ($data)
                {
                    $sms = new fondaSMS_OutgoingMessage();
                    $sms->id = $data['id'];
                    $sms->to = $data['to'];
        ...
        if ($messages)
        {
            $events[] = new fondaSMS_Event_Send($messages);
        }
