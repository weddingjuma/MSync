...
/*
 * PHP server library for fondaSMS 3.0
 *
 ...
 class fondaSMS
{
    ...
    static function get_request()
    {
        if (!isset(self::$request))
        {
            ...
            {
                self::$request = new fondaSMS_ActionRequest();
            }
            else
            {
                self::$request = new fondaSMS_Request();
            }
...
class fondaSMS_Request
{
    ...
    function render_response($events = null /* optional array of fondaSMS_Event objects */)
    {
        if ($this->supports_json())
        {
        ...
        else
        {
            ...
            if ($events)
            {
                ...
                {
                    echo "<messages>";
                    if ($event instanceof fondaSMS_Event_Send)
                    {
                        if ($event->messages)
                        {
                            foreach ($event->messages as $message)
                            {
                                $type = isset($message->type) ? $message->type : fondaSMS::MESSAGE_TYPE_SMS;
                                $id = isset($message->id) ? " id=\"".fondaSMS::escape($message->id)."\"" : "";
                                $to = isset($message->to) ? " to=\"".fondaSMS::escape($message->to)."\"" : "";
                                $priority = isset($message->priority) ? " priority=\"".$message->priority."\"" : "";
                                echo "<$type$id$to$priority>".fondaSMS::escape($message->message)."</$type>";
                            }
                        }
                    }
                    echo "</messages>";
...
class fondaSMS_ActionRequest extends fondaSMS_Request
{
    ...
    public $power;          // power source integer, see fondaSMS::POWER_SOURCE_*
    ...
    private function _get_action()
    {
        switch (@$_POST['action'])
        {
            case fondaSMS::ACTION_INCOMING:
                return new fondaSMS_Action_Incoming($this);
            case fondaSMS::ACTION_FORWARD_SENT:
                return new fondaSMS_Action_ForwardSent($this);
            case fondaSMS::ACTION_OUTGOING:
                return new fondaSMS_Action_Outgoing($this);
            case fondaSMS::ACTION_SEND_STATUS:
                return new fondaSMS_Action_SendStatus($this);
            case fondaSMS::ACTION_TEST:
                return new fondaSMS_Action_Test($this);
            case fondaSMS::ACTION_DEVICE_STATUS:
                return new fondaSMS_Action_DeviceStatus($this);
            case fondaSMS::ACTION_AMQP_STARTED:
                return new fondaSMS_Action_AmqpStarted($this);
            default:
                return new fondaSMS_Action($this);
        }
    }
...
class fondaSMS_OutgoingMessage
{
    ...
    public $type;            // fondaSMS::MESSAGE_TYPE_* value (default sms)
}
...
abstract class fondaSMS_Action_Forward extends fondaSMS_Action
{
    public $message;        // The message body of the SMS, or the content of the text/plain part of the MMS.
    public $message_type;   // fondaSMS::MESSAGE_TYPE_MMS or fondaSMS::MESSAGE_TYPE_SMS
    public $mms_parts;      // array of fondaSMS_MMS_Part instances
    ...
    function __construct($request)
    {
        ...
        if ($this->message_type == fondaSMS::MESSAGE_TYPE_MMS)
        {
            $this->mms_parts = array();
            foreach (json_decode($_POST['mms_parts'], true) as $mms_part)
            {
                $this->mms_parts[] = new fondaSMS_MMS_Part($mms_part);
            }
        }
    }
}

class fondaSMS_Action_Incoming extends fondaSMS_Action_Forward
{
    ...
    {
        parent::__construct($request);
        $this->type = fondaSMS::ACTION_INCOMING;
        $this->from = $_POST['from'];
    }
}

class fondaSMS_Action_ForwardSent extends fondaSMS_Action_Forward
{
    ...
    {
        parent::__construct($request);
        $this->type = fondaSMS::ACTION_FORWARD_SENT;
        $this->to = $_POST['to'];
    }
}

class fondaSMS_Action_AmqpStarted extends fondaSMS_Action
{
    public $consumer_tag;
    ...
    {
        parent::__construct($request);
        $this->type = fondaSMS::ACTION_AMQP_STARTED;
        $this->consumer_tag = $_POST['consumer_tag'];
    }
}

class fondaSMS_Action_Outgoing extends fondaSMS_Action
{
    function __construct($request)
    {
        parent::__construct($request);
        $this->type = fondaSMS::ACTION_OUTGOING;
    }
}

class fondaSMS_Action_Test extends fondaSMS_Action
{
    function __construct($request)
    {
        parent::__construct($request);
        $this->type = fondaSMS::ACTION_TEST;
    }
}

class fondaSMS_Action_SendStatus extends fondaSMS_Action
{
    public $status;     // fondaSMS::STATUS_* values
    public $id;         // server ID previously used in fondaSMS_OutgoingMessage
    public $error;      // textual description of error (if applicable)

    function __construct($request)
    {
        parent::__construct($request);
        $this->type = fondaSMS::ACTION_SEND_STATUS;
...
class fondaSMS_Action_DeviceStatus extends fondaSMS_Action
{
    public $status;     // fondaSMS::DEVICE_STATUS_* values

    function __construct($request)
    {
        parent::__construct($request);
        $this->type = fondaSMS::ACTION_DEVICE_STATUS;
        $this->status = $_POST['status'];
    }
}
 ...
 */
class fondaSMS_Event_Send extends fondaSMS_Event
{
    public $messages;

    function __construct($messages /* array of fondaSMS_OutgoingMessage objects */)
    {
        $this->event = fondaSMS::EVENT_SEND;
 ...
 */
class fondaSMS_Event_Cancel extends fondaSMS_Event
{
    public $id;

    function __construct($id /* id of previously created fondaSMS_OutgoingMessage object (string) */)
    {
        $this->event = fondaSMS::EVENT_CANCEL;
        $this->id = $id;
    }
}
 ...
 */
class fondaSMS_Event_CancelAll extends fondaSMS_Event
{
    function __construct()
    {
        $this->event = fondaSMS::EVENT_CANCEL_ALL;
    }
}
 ...
 */
class fondaSMS_Event_Log extends fondaSMS_Event
{
    public $message;

    function __construct($message)
    {
        $this->event = fondaSMS::EVENT_LOG;
